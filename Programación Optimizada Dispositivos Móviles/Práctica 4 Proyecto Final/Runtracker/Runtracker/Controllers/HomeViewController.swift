...
class HomeViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func saveTraining() {
        ...
        if let trainingsData = userDefauls.object(forKey: "trainings") as? Data {
            ...
            let train = Train(date: dateFormatter.string(from: Date()), distance: self.distanceLabel.text!, time: self.chronometerLabel.text!, calories: calories)
    ...
    func calculateCalories() -> Int {
        ...
        let weight = userDefauls.string(forKey: "weight")
        if let savedWeight = weight {
            let integerWeight = Int(savedWeight) ?? 0
            return Int(self.met * Int(0.0175) * integerWeight)
        }
        else {
            return 0
        }
